...
function New-OrResetAppPool {
    ...
    & $AppCmd set apppool /apppool.name:$Name /managedRuntimeVersion:v4.0 /managedPipelineMode:Integrated /processModel.identityType:ApplicationPoolIdentity /processModel.maxProcesses:1 /recycling.disallowOverlappingRotation:true | Out-Null
    Assert-AppPoolNoOverlappingRotation -AppCmd $AppCmd -Name $Name
    & $AppCmd start apppool /apppool.name:$Name 2>$null | Out-Null
}

function Assert-AppPoolNoOverlappingRotation {
    param(
        [string]$AppCmd,
        [string]$Name
    )

    $value = & $AppCmd list apppool "/apppool.name:$Name" /text:recycling.disallowOverlappingRotation 2>$null | Select-Object -First 1
    if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace([string]$value) -or -not [bool]::Parse(([string]$value).Trim())) {
        throw ("Application pool {0} must have recycling.disallowOverlappingRotation=true." -f $Name)
    }
}
