...
namespace GatewayDemo.Legacy.Core.Options
{
    public static class GatewayPathRuleProfileCatalog
    {
        ...
        private static bool ApplyConfiguredProfile(GatewaySiteOptions site, string profileName)
        {
            ...
            foreach (var rule in profile.PathRules ?? new List<GatewayPathRuleOptions>())
            {
                AddOrMergeRule(
                    ...
                    rule.ReadWriteTimeoutMilliseconds);
            }
        ...
        private static void AddOrMergeRule(
            GatewaySiteOptions site,
            string name,
            IEnumerable<string> patterns,
            bool resolveFromUpstreamOriginRoot,
            int timeoutSeconds,
            int readWriteTimeoutSeconds,
            bool treatAsLongPolling,
            bool treatAsStreaming,
            int timeoutMilliseconds = 0,
            int readWriteTimeoutMilliseconds = 0)
        {
            ...
            if (rule == null)
            {
                rule = new GatewayPathRuleOptions
                {
                    ...
                    TimeoutSeconds = timeoutMilliseconds > 0 ? 0 : timeoutSeconds,
                    ReadWriteTimeoutSeconds = readWriteTimeoutMilliseconds > 0 ? 0 : readWriteTimeoutSeconds,
                    TimeoutMilliseconds = timeoutMilliseconds,
                    ReadWriteTimeoutMilliseconds = readWriteTimeoutMilliseconds
                };
            ...
            else
            {
                ...
                if (rule.TimeoutSeconds <= 0 && rule.TimeoutMilliseconds <= 0)
                {
                    rule.TimeoutSeconds = timeoutMilliseconds > 0 ? 0 : timeoutSeconds;
                    rule.TimeoutMilliseconds = timeoutMilliseconds;
                }

                if (rule.ReadWriteTimeoutSeconds <= 0 && rule.ReadWriteTimeoutMilliseconds <= 0)
                {
                    rule.ReadWriteTimeoutSeconds = readWriteTimeoutMilliseconds > 0 ? 0 : readWriteTimeoutSeconds;
                    rule.ReadWriteTimeoutMilliseconds = readWriteTimeoutMilliseconds;
                }
            }
