...
namespace GatewayDemo.Legacy.Web.Infrastructure
{
    public sealed class LegacyGatewayRepository : IGatewayRepository
    {
        ...
        private const int DatabaseWriteWallClockBudgetMilliseconds = 2000;
        private const int DatabaseWriteBusyTimeoutMilliseconds = 50;
        ...
        [ThreadStatic]
        private static bool useBestEffortWriteConnection;
...
WHERE DeviceId = @DeviceId;";
                        command.Parameters.AddWithValue("@RevokedAtUtc", ToDbDateTime(now));
                        command.Parameters.AddWithValue("@RevokedReason", "authorization-revoked");
                        command.Parameters.AddWithValue("@DeviceId", authorization.DeviceId);
                        command.ExecuteNonQuery();
                    }
                }
                ...
                using (var command = connection.CreateCommand())
                {
                    command.Transaction = transaction;
                    command.CommandText = @"
UPDATE GatewayWebViewCredentials
SET RevokedAtUtc = @RevokedAtUtc
WHERE DeviceId = @DeviceId AND RevokedAtUtc IS NULL;
DELETE FROM GatewayWebViewHandoffTickets WHERE DeviceId = @DeviceId;";
                    command.Parameters.AddWithValue("@RevokedAtUtc", ToDbDateTime(now));
                    command.Parameters.AddWithValue("@DeviceId", authorization.DeviceId);
                    command.ExecuteNonQuery();
                }
        ...
        public void CreateWebViewHandoffTicket(GatewayWebViewHandoffTicket ticket)
        {
            if (ticket == null || string.IsNullOrWhiteSpace(ticket.TicketHash))
            {
            ...
            ExecuteDatabaseWrite(delegate
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var delete = connection.CreateCommand())
                    {
                        ...
                        delete.CommandText = "DELETE FROM GatewayWebViewHandoffTickets WHERE DeviceId = @DeviceId AND SiteKey = @SiteKey AND (ConsumedAtUtc IS NOT NULL OR ExpiresAtUtc <= @Now);";
                        delete.Parameters.AddWithValue("@DeviceId", ticket.DeviceId ?? string.Empty);
                        delete.Parameters.AddWithValue("@SiteKey", ticket.SiteKey ?? string.Empty);
                        delete.Parameters.AddWithValue("@Now", ToDbDateTime(DateTime.UtcNow));
                    ...
                    using (var command = connection.CreateCommand())
                    {
                        command.Transaction = transaction;
                        command.CommandText = @"
INSERT INTO GatewayWebViewHandoffTickets
(Id, TicketHash, DeviceId, SiteKey, HostBinding, TargetPath, ExpiresAtUtc, ConsumedAtUtc, CreatedAtUtc)
VALUES (@Id, @TicketHash, @DeviceId, @SiteKey, @HostBinding, @TargetPath, @ExpiresAtUtc, NULL, @CreatedAtUtc);";
                        command.Parameters.AddWithValue("@Id", ticket.Id ?? Guid.NewGuid().ToString("D"));
                        command.Parameters.AddWithValue("@TicketHash", ticket.TicketHash);
                        command.Parameters.AddWithValue("@DeviceId", ticket.DeviceId ?? string.Empty);
                        command.Parameters.AddWithValue("@SiteKey", ticket.SiteKey ?? string.Empty);
                        command.Parameters.AddWithValue("@HostBinding", ticket.HostBinding ?? string.Empty);
                        command.Parameters.AddWithValue("@TargetPath", ticket.TargetPath ?? string.Empty);
                        command.Parameters.AddWithValue("@ExpiresAtUtc", ToDbDateTime(ticket.ExpiresAtUtc));
                        command.Parameters.AddWithValue("@CreatedAtUtc", ToDbDateTime(ticket.CreatedAtUtc));
                        command.ExecuteNonQuery();
                    }
        ...
        public GatewayWebViewHandoffTicket GetActiveWebViewHandoffTicket(string ticketHash, string siteKey, string hostBinding, DateTime now)
        {
            using (var connection = OpenConnection())
            using (var command = connection.CreateCommand())
            {
                command.CommandText = @"
SELECT * FROM GatewayWebViewHandoffTickets
WHERE TicketHash = @TicketHash AND SiteKey = @SiteKey AND HostBinding = @HostBinding
  AND ConsumedAtUtc IS NULL AND ExpiresAtUtc > @Now
LIMIT 1;";
                command.Parameters.AddWithValue("@TicketHash", ticketHash ?? string.Empty);
                command.Parameters.AddWithValue("@SiteKey", siteKey ?? string.Empty);
                command.Parameters.AddWithValue("@HostBinding", hostBinding ?? string.Empty);
                command.Parameters.AddWithValue("@Now", ToDbDateTime(now));
                using (var reader = command.ExecuteReader())
                {
                    return reader.Read() ? MapWebViewHandoffTicket(reader) : null;
                }
            }
        }

        public GatewayWebViewCredential ConsumeWebViewHandoffTicket(string ticketHash, string siteKey, string hostBinding, DateTime now, string tokenHash, DateTime credentialExpiresAtUtc)
        {
            if (string.IsNullOrWhiteSpace(ticketHash) || string.IsNullOrWhiteSpace(siteKey) || string.IsNullOrWhiteSpace(hostBinding))
            {
                return null;
            }
            ...
            return ExecuteDatabaseWrite(delegate
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    GatewayWebViewHandoffTicket ticket = null;
                    using (var command = connection.CreateCommand())
                    {
                        command.Transaction = transaction;
                        command.CommandText = @"
...
LIMIT 1;";
                        ...
                        command.Parameters.AddWithValue("@HostBinding", hostBinding);
                        command.Parameters.AddWithValue("@Now", ToDbDateTime(now));
                        using (var reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ticket = MapWebViewHandoffTicket(reader);
                            }
                        }
                    }
                    ...
                    using (var consume = connection.CreateCommand())
                    {
                        ...
                        consume.CommandText = "UPDATE GatewayWebViewHandoffTickets SET ConsumedAtUtc = @ConsumedAtUtc WHERE Id = @Id AND ConsumedAtUtc IS NULL;";
                        consume.Parameters.AddWithValue("@ConsumedAtUtc", ToDbDateTime(now));
                        consume.Parameters.AddWithValue("@Id", ticket.Id);
                    ...
                    using (var revoke = connection.CreateCommand())
                    {
                        ...
                        revoke.CommandText = "DELETE FROM GatewayWebViewCredentials WHERE DeviceId = @DeviceId AND SiteKey = @SiteKey;";
                        revoke.Parameters.AddWithValue("@RevokedAtUtc", ToDbDateTime(now));
                        revoke.Parameters.AddWithValue("@DeviceId", ticket.DeviceId);
                        revoke.Parameters.AddWithValue("@SiteKey", ticket.SiteKey);
                        revoke.ExecuteNonQuery();
                    }

                    var credential = new GatewayWebViewCredential
                    {
                        Id = Guid.NewGuid().ToString("D"),
                        TokenHash = tokenHash,
                        DeviceId = ticket.DeviceId,
                        SiteKey = ticket.SiteKey,
                        HostBinding = ticket.HostBinding,
                    ...
                    using (var insert = connection.CreateCommand())
                    {
                        ...
                        insert.CommandText = @"
INSERT INTO GatewayWebViewCredentials
(Id, TokenHash, DeviceId, SiteKey, HostBinding, TargetPath, ExpiresAtUtc, RevokedAtUtc, CreatedAtUtc)
VALUES (@Id, @TokenHash, @DeviceId, @SiteKey, @HostBinding, @TargetPath, @ExpiresAtUtc, NULL, @CreatedAtUtc);";
                        insert.Parameters.AddWithValue("@Id", credential.Id);
                        insert.Parameters.AddWithValue("@TokenHash", credential.TokenHash);
                        insert.Parameters.AddWithValue("@DeviceId", credential.DeviceId);
                        insert.Parameters.AddWithValue("@SiteKey", credential.SiteKey);
                        insert.Parameters.AddWithValue("@HostBinding", credential.HostBinding);
                        insert.Parameters.AddWithValue("@TargetPath", credential.TargetPath ?? string.Empty);
                        insert.Parameters.AddWithValue("@ExpiresAtUtc", ToDbDateTime(credential.ExpiresAtUtc));
                        insert.Parameters.AddWithValue("@CreatedAtUtc", ToDbDateTime(credential.CreatedAtUtc));
        ...
        public GatewayWebViewCredential GetActiveWebViewCredential(string tokenHash, string siteKey, string hostBinding, DateTime now)
        {
            using (var connection = OpenConnection())
            using (var command = connection.CreateCommand())
            {
                command.CommandText = @"
SELECT * FROM GatewayWebViewCredentials
WHERE TokenHash = @TokenHash AND SiteKey = @SiteKey AND HostBinding = @HostBinding
  AND RevokedAtUtc IS NULL AND ExpiresAtUtc > @Now
LIMIT 1;";
                command.Parameters.AddWithValue("@TokenHash", tokenHash ?? string.Empty);
                command.Parameters.AddWithValue("@SiteKey", siteKey ?? string.Empty);
                command.Parameters.AddWithValue("@HostBinding", hostBinding ?? string.Empty);
                command.Parameters.AddWithValue("@Now", ToDbDateTime(now));
                using (var reader = command.ExecuteReader())
                {
                    return reader.Read() ? MapWebViewCredential(reader) : null;
                }
            }
        }

        public void RevokeWebViewCredentials(string deviceId)
        {
            ...
            ExecuteDatabaseWrite(delegate
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    ...
                    {
                        command.Transaction = transaction;
                        command.CommandText = "UPDATE GatewayWebViewCredentials SET RevokedAtUtc = @RevokedAtUtc WHERE DeviceId = @DeviceId AND RevokedAtUtc IS NULL; DELETE FROM GatewayWebViewHandoffTickets WHERE DeviceId = @DeviceId;";
                        command.Parameters.AddWithValue("@RevokedAtUtc", ToDbDateTime(DateTime.UtcNow));
                        command.Parameters.AddWithValue("@DeviceId", deviceId);
                        command.ExecuteNonQuery();
                    }
                    transaction.Commit();
        ...
        public Task TouchAuthorizationAsync(string deviceId, string clientIp, string siteKey, string requestPath, CancellationToken cancellationToken)
        {
            var now = DateTime.UtcNow;
            var heartbeatKey = "authorization:" + deviceId + ":" + siteKey;
            if (!ShouldWriteHeartbeat(heartbeatKey, now))
            {
                return Task.CompletedTask;
            }

            var safePath = SanitizeTargetPath(requestPath);
            QueueBestEffortDatabaseWrite(delegate
            {
...
WHERE DeviceId = @DeviceId AND Status = @Status;";
        ...
        public Task RecordAuditAsync(string kind, string message, DeviceContext device, string siteKey, string clientIp, CancellationToken cancellationToken)
        {
            ...
            QueueBestEffortDatabaseWrite(delegate
            {
                using (var connection = OpenConnection(true))
                {
                    InsertAudit(connection, null, kind, message, deviceId, deviceCode, siteKey, clientIp, DateTime.UtcNow);
                }
            });
            return Task.CompletedTask;
        }
        ...
        private SQLiteConnection OpenConnection(bool bestEffortWrite = false)
        {
            EnsureDatabaseFileExists();
            bestEffortWrite = bestEffortWrite || useBestEffortWriteConnection;
            var connection = new SQLiteConnection(configuration.Storage.SqliteConnectionString);
            connection.Open();
            SQLiteConnectionConfigurator.ConfigureOpenConnection(connection, bestEffortWrite ? DatabaseWriteBusyTimeoutMilliseconds : (int?)null);
            return connection;
        }
        ...
        private static T ExecuteDatabaseWrite<T>(Func<T> action)
        {
            ...
            var previousUseBestEffortWriteConnection = useBestEffortWriteConnection;
            useBestEffortWriteConnection = true;
            try
            {
                ...
                for (var attempt = 1; ; attempt++)
                {
                    try
                    {
                    var lockWaitStarted = Stopwatch.StartNew();
                    var lockTaken = false;
                    try
                    {
                        var remainingMilliseconds = DatabaseWriteWallClockBudgetMilliseconds - (int)started.ElapsedMilliseconds;
                        if (remainingMilliseconds <= 0 || !Monitor.TryEnter(DatabaseWriteSyncRoot, remainingMilliseconds))
                        {
                            Trace.TraceWarning(
                                "Gateway database write lock wait exceeded budget: waitMs={0}, totalMs={1}",
                                lockWaitStarted.ElapsedMilliseconds,
                                started.ElapsedMilliseconds);
                            throw new TimeoutException("The gateway database write lock exceeded its wall-clock budget.");
                        }

                        lockTaken = true;
                        var databaseStarted = Stopwatch.StartNew();
                        try
                        {
                            return action();
                        }
                        ...
                        {
                            var lockWaitMilliseconds = lockWaitStarted.ElapsedMilliseconds;
                            var databaseMilliseconds = databaseStarted.ElapsedMilliseconds;
                            if (lockWaitMilliseconds >= 100 || databaseMilliseconds >= 100)
                            {
                                Trace.TraceWarning(
                                    "Gateway database write timing: lockWaitMs={0}, dbMs={1}, totalMs={2}",
                    ...
                    finally
                    {
                        if (lockTaken)
                        {
                            Monitor.Exit(DatabaseWriteSyncRoot);
                        }
                    }
                    }
                    catch (SQLiteException ex)
                    {
                        ...
                        var remainingRetryMilliseconds = DatabaseWriteWallClockBudgetMilliseconds - (int)started.ElapsedMilliseconds;
                        if (remainingRetryMilliseconds <= 0)
                        {
                            throw;
                        }

                        var retryDelay = GetRetryDelay(attempt);
                        Thread.Sleep(retryDelay.TotalMilliseconds > remainingRetryMilliseconds
                            ? remainingRetryMilliseconds
            ...
            {
                useBestEffortWriteConnection = previousUseBestEffortWriteConnection;
            }
        }

        private void QueueBestEffortDatabaseWrite(Action action)
        {
            if (action == null)
            {
                return;
            ...
            ThreadPool.QueueUserWorkItem(delegate
            {
                try
                {
                    if (!Monitor.TryEnter(DatabaseWriteSyncRoot))
                    {
                        return;
                    }
                ...
                {
                    // Heartbeat/audit writes are diagnostic best-effort work and must not
                    // turn a completed business request into a gateway error.
                }
                catch (IOException)
                {
                }
                ...
                {
                    // Diagnostic writes must never become unobserved request failures.
                }
            });
        }

        private static string SanitizeTargetPath(string requestPath)
        {
        ...
        private static GatewayWebViewHandoffTicket MapWebViewHandoffTicket(IDataRecord record)
        {
            return new GatewayWebViewHandoffTicket
            {
                Id = Convert.ToString(record["Id"]) ?? string.Empty,
                TicketHash = Convert.ToString(record["TicketHash"]) ?? string.Empty,
                DeviceId = Convert.ToString(record["DeviceId"]) ?? string.Empty,
                SiteKey = Convert.ToString(record["SiteKey"]) ?? string.Empty,
                HostBinding = Convert.ToString(record["HostBinding"]) ?? string.Empty,
                TargetPath = Convert.ToString(record["TargetPath"]) ?? string.Empty,
                ExpiresAtUtc = ParseDbDateTime(record["ExpiresAtUtc"]),
                ConsumedAtUtc = ParseNullableDbDateTime(record["ConsumedAtUtc"]),
                CreatedAtUtc = ParseDbDateTime(record["CreatedAtUtc"])
            };
        }

        private static GatewayWebViewCredential MapWebViewCredential(IDataRecord record)
        {
            return new GatewayWebViewCredential
            {
                Id = Convert.ToString(record["Id"]) ?? string.Empty,
                TokenHash = Convert.ToString(record["TokenHash"]) ?? string.Empty,
                DeviceId = Convert.ToString(record["DeviceId"]) ?? string.Empty,
                SiteKey = Convert.ToString(record["SiteKey"]) ?? string.Empty,
                HostBinding = Convert.ToString(record["HostBinding"]) ?? string.Empty,
                TargetPath = Convert.ToString(record["TargetPath"]) ?? string.Empty,
        ...
        private static GatewayAppCredential MapAppCredential(IDataRecord record)
        {
            return new GatewayAppCredential
            {
                ...
                RevokedAtUtc = ParseNullableDbDateTime(record["RevokedAtUtc"]),
                RevokedReason = Convert.ToString(record["RevokedReason"]) ?? string.Empty
            };
        }
...
VALUES (@Id, @AuthorizationId, @SiteKey);";
        ...
        private static GatewayDeviceAuthorization MapAuthorization(IDataRecord record)
        {
            ...
            return new GatewayDeviceAuthorization
            {
                ...
                LastSeenPath = Convert.ToString(record["LastSeenPath"]) ?? string.Empty,
                CreatedAtUtc = ParseDbDateTime(record["CreatedAtUtc"]),
        ...
        private static void BindAuthorization(SQLiteCommand command, GatewayDeviceAuthorization authorization)
        {
            ...
            command.Parameters.AddWithValue("@LastSeenPath", (object)authorization.LastSeenPath ?? DBNull.Value);
...
INSERT INTO GatewayDeviceAuthorizations (
    ...
    ReviewNote, LastSeenIp, LastSeenSiteKey, LastSeenPath, CreatedAtUtc, ApprovedAtUtc, LastSeenAtUtc, RevokedAtUtc
) VALUES (
    @Id, @DeviceId, @DeviceCode, @CompanyName, @ApplicantName, @Phone, @LegacyCompanyId, @LegacyUrlBefore,
    @LegacyUserId, @LegacyDataCenterId, @LegacyDeviceImei, @LegacyMobileUserNum, @Status, @PolicyMode,
    @ReviewNote, @LastSeenIp, @LastSeenSiteKey, @LastSeenPath, @CreatedAtUtc, @ApprovedAtUtc, @LastSeenAtUtc, @RevokedAtUtc
