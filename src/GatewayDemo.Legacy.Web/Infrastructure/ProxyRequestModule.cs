    ...
    public sealed class ProxyRequestModule : IHttpModule
    {
        ...
        private static void HandleRequest(object sender, EventArgs args)
        {
            ...
            catch (DeviceCredentialException ex)
            {
                ...
                var isExpiredLegacySessionLogin = ex.StatusCode == 401
                    && descriptor.IsLegacyApp
                    && descriptor.IsLoginRequest
                    && descriptor.HasSessionKey
                    && !descriptor.HasIdentity;
                var isNativeAppRequest = descriptor.IsLegacyApp
                    || HasAnyAppCredentialHeaders(context.Request)
                    || (MobileClientDetector.IsNativeAppHealthProbeRequest(context.Request)
                        && !IsBrowserDocumentNavigation(context.Request));
                if (isExpiredLegacySessionLogin)
                {
                    LegacyAppResponseWriter.WriteBlockedJson(
                        context,
                        401,
                        "移动 APP 登录会话已失效，请重新登录。",
                        descriptor,
                        "session_expired",
                        false,
                        true,
            ...
            if (device.RequiresReview || authorization == null || !authorization.AllowsSite(siteKey))
            {
                ...
                if (credentialService.IsLegacyAppContext(device) || descriptor.IsLegacyApp || LegacyAppCompatibilityPolicy.LooksLikeMobileAppRequest(context, site))
                {
                    ...
                    var legacyBlockMessage = BuildLegacyAppBlockMessage(device, authorization, siteKey);
                    if (IsAuthorizationRevoked(device))
                    {
                        LegacyAppResponseWriter.WriteBlockedJson(
                            context,
                            403,
            ...
            repository.TouchAuthorizationAsync(device.DeviceId, device.ClientIp, siteKey, requestPath, CancellationToken.None)
                .GetAwaiter()
                .GetResult();
        ...
        private static bool HasAnyAppCredentialHeaders(HttpRequest request)
        {
            if (request == null)
            {
                return false;
            ...
            string forcedRelativePath)
        {
            ...
            if (LooksLikeStaticSubResourceRequest(context, context.Request.Path ?? string.Empty))
            {
                var profile = LegacyAppCompatibilityPolicy.GetProfile(site);
                foreach (var pattern in profile.AnonymousStaticSubResourcePathPatterns ?? new List<string>())
                {
                    if (MatchesAnyAllowedPath(candidates, pattern))
                    {
                        return true;
            ...
            if (site.AnonymousAllowedPaths == null || site.AnonymousAllowedPaths.Count == 0)
            {
                return false;
            }
        ...
        private static bool IsGatewayOwnPath(string requestPath)
        {
            ...
            if (string.Equals(requestPath, "/healthz.ashx", StringComparison.OrdinalIgnoreCase)
                || string.Equals(requestPath, "/Gateway/WebViewHandoff.ashx", StringComparison.OrdinalIgnoreCase))
            {
                return true;
            }
        ...
        private static bool IsGatewayReservedPath(string requestPath)
        {
            ...
            return path.StartsWith("/gateway", StringComparison.OrdinalIgnoreCase)
                ...
                || string.Equals(path, "/Gateway/WebViewHandoff.ashx", StringComparison.OrdinalIgnoreCase);
        }
        ...
        private static bool IsAuthorizationRevoked(DeviceContext device)
        {
            return device != null
                && device.RequiresReview
                && !string.IsNullOrWhiteSpace(device.ChallengeReason)
                && device.ChallengeReason.IndexOf("撤销", StringComparison.OrdinalIgnoreCase) >= 0;
        }
