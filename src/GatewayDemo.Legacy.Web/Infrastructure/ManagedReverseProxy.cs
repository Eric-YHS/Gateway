    ...
    public sealed class ManagedReverseProxy
    {
        ...
        public void Proxy(HttpContext context, GatewaySiteOptions site, DeviceContext device, DeviceCredentialService credentialService)
        {
            ...
            catch (WebException ex)
            {
                ...
                {
                    if (IsClientDisconnectAbort(context, ex))
                    {
                        LogClientDisconnected(context, ex);
                        return;
                    }
            ...
            catch (GatewayClientDisconnectedException ex)
            {
                LogClientDisconnected(context, ex);
                return;
            }
        ...
        private static void LogClientDisconnected(HttpContext context, Exception exception)
        {
            var rawUrl = context == null || context.Request == null
                ? "(unknown)"
                : context.Request.RawUrl;
            Trace.TraceWarning(
                "GatewayDemo ClientDisconnected: path={0}, exception={1}: {2}",
                rawUrl ?? "(unknown)",
        ...
        private static int ResolveTimeoutMilliseconds(int configuredMilliseconds, int configuredSeconds, int fallbackMilliseconds)
        {
        ...
        private static void WriteRequestBody(HttpContext context, HttpWebRequest request)
        {
            ...
            catch (HttpException ex)
            {
                ...
                {
                    throw new GatewayClientDisconnectedException(ex);
                }
            ...
            try
            {
                ...
                {
                    inputStream.CopyTo(upstreamStream);
                }
            }
            catch (HttpException ex)
            {
        ...
        {
            if (exception is GatewayClientDisconnectedException)
            {
                return true;
            }
        ...
        private static bool IsClientAbortException(Exception exception)
        {
            ...
            while (current != null)
            {
                if (current is GatewayClientDisconnectedException)
                {
                    return true;
                }
                ...
                if (current.HResult == unchecked((int)0x800703E3)
                    || current.HResult == unchecked((int)0x80070040)
                    || current.HResult == unchecked((int)0x800704D4))
                {
                    return true;
                }
