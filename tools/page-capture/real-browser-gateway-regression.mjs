...
const gatewayWebConfigPath = path.join(root, 'src', 'GatewayDemo.Legacy.Web', 'Web.config');
const gatewaySitesPath = path.join(root, 'src', 'GatewayDemo.Legacy.Web', 'App_Data', 'gateway-sites.json');
const gatewayAppData = path.join(root, 'src', 'GatewayDemo.Legacy.Web', 'App_Data');
...
async function writeApplicationHostConfig() {
  ...
  const gatewayPath = xmlEscape(path.join(root, 'src', 'GatewayDemo.Legacy.Web'));
  const backendPath = xmlEscape(path.join(root, 'src', 'MockBusinessBackend.Legacy.Web'));
  const sitesXml = `
        <sites>
...
async function runBrowserScenario() {
  ...
  const company = `RBTest-${Date.now()}`;
  const reviewedSitesLabel = '\u5ba1\u6838\u901a\u8fc7\u7ad9\u70b9';
  const allSitesText = '\u6240\u6709\u7ad9\u70b9';
  ...
  try {
    ...
    recordCheck('initial ERP visit is blocked for authorization request', await page.locator('input[name="companyName"]').count() === 1);

    await page.fill('input[name="companyName"]', company);
    await page.fill('input[name="applicantName"]', 'Real Browser Tester');
    await page.fill('input[name="phone"]', '13800138000');
    await page.fill('input[name="reason"]', 'real-browser-regression');
    await page.check('form input.js-all-sites[name="allowAllSites"]');
    await assertAllSitesState(page.locator('form').filter({ has: page.locator('input[name="companyName"]') }).first(), true, true, 0, 'applicant selected all-sites');
    await screenshot(page, 'applicant-all-sites-checked');
    await Promise.all([
      page.waitForLoadState('networkidle'),
      page.click('form button[type="submit"]')
    ]);
    await screenshot(page, 'applicant-request-submitted');
    ...
    await assertAllSitesState(pending, true, true, 0, 'admin pending all-sites request');

    await pending.locator('input.js-all-sites').uncheck();
    await pending.locator('input[name="siteKeys"][value="erp-main"]').uncheck();
    await pending.locator('input[name="siteKeys"][value="wms-east"]').check();
    await assertAllSitesState(pending, false, false, 1, 'admin modified request to WMS only');
    await pending.locator('input[name="note"]').fill('approve WMS only');
    await screenshot(admin, 'admin-modified-sites-before-approve');
    await Promise.all([
      admin.waitForLoadState('networkidle'),
      pending.locator('button[name="action"][value="approve-request"]').click()
    ]);
    await screenshot(admin, 'admin-after-wms-only-approval');

    await page.goto(`http://${hosts.erp}/Gateway/Default.aspx?target=/portal`, { waitUntil: 'networkidle' });
    await screenshot(page, 'applicant-reviewed-sites-wms-only');
    const reviewedSitesValue = await page.evaluate(label => {
      const rows = [...document.querySelectorAll('.grid div')].map(row => ({
        label: row.querySelector('span')?.textContent?.trim() ?? '',
        value: row.querySelector('strong')?.textContent?.trim() ?? ''
      }));
      return rows.find(row => row.label === label)?.value ?? '';
    }, reviewedSitesLabel);
    recordCheck('applicant sees reviewed sites row', reviewedSitesValue.length > 0, reviewedSitesValue);
    recordCheck('applicant reviewed sites reflect admin-modified WMS only', reviewedSitesValue.includes('WMS East') && !reviewedSitesValue.includes(allSitesText), reviewedSitesValue);

    await page.goto(`http://${hosts.erp}/portal`, { waitUntil: 'networkidle' });
    await screenshot(page, 'erp-still-blocked-after-wms-only');
    recordCheck('ERP remains blocked after WMS-only approval', await page.locator('input[name="companyName"]').count() === 1);

    await page.goto(`http://${hosts.wms}/portal`, { waitUntil: 'networkidle' });
    await screenshot(page, 'wms-authorized-cross-domain');
    recordCheck('WMS cross-domain site is authorized', await page.locator('input[name="companyName"]').count() === 0);
    recordCheck('WMS upstream page is rendered', (await page.textContent('body')).includes('WMS'));

    const authArticle = admin.locator('article.item').filter({ hasText: company }).filter({ has: admin.locator('input[name="authorizationId"]') }).first();
    await authArticle.waitFor({ state: 'visible', timeout: 15000 });
    const updateForm = authArticle.locator('form').filter({ has: admin.locator('input[name="authorizationId"]') }).first();
    await updateForm.locator('input.js-all-sites').check();
    await assertAllSitesState(updateForm, true, true, 0, 'admin update authorization to all-sites');
    await updateForm.locator('input[name="note"]').fill('update to all sites');
    await screenshot(admin, 'admin-update-authorization-all-sites');
    await Promise.all([
      admin.waitForLoadState('networkidle'),
      updateForm.locator('button[type="submit"]').click()
    ]);
    await screenshot(admin, 'admin-after-all-sites-update');

    await page.goto(`http://${hosts.finance}/portal`, { waitUntil: 'networkidle' });
    await screenshot(page, 'finance-authorized-after-all-sites');
    recordCheck('all-sites authorization is shared across domains', await page.locator('input[name="companyName"]').count() === 0);
    recordCheck('Finance request reaches upstream login instead of gateway approval', await page.locator('input[name="username"]').count() === 1);

    const cookies = await context.cookies();
    const deviceCookie = cookies.find(cookie => cookie.name === 'gw_device_credential');
    recordCheck('device cookie uses parent domain for cross-domain cache', !!deviceCookie && deviceCookie.domain === parentCookieDomain, JSON.stringify(deviceCookie ?? {}));

    await page.goto(`http://${hosts.wms}/login?returnUrl=portal`, { waitUntil: 'networkidle' });
    const wsResult = await page.evaluate(port => new Promise(resolve => {
      const socket = new WebSocket(`ws://b.gwdemo.test:${port}/socket`);
      ...
      socket.onmessage = event => {
        ...
        socket.close();
        resolve({ ok: data === 'echo:ping', data });
      };
      socket.onerror = () => {
        clearTimeout(timer);
    ...
    recordCheck('WebSocket proxy echoes through browser', wsResult.ok, JSON.stringify(wsResult));

    await writeGatewaySitesConfig({ redirectSelf: true });
    await stopIisExpressProcesses();
    spawnIisExpress('GatewayRealBrowser', 'gateway-redirect-self');
    await waitForUrl(`http://localhost:${gatewayPort}/healthz.ashx`);

    await page.goto(`http://${hosts.erp}/portal`, { waitUntil: 'networkidle' });
    await screenshot(page, 'redirect-to-upstream-self-protected');
    const body = await page.textContent('body');
    recordCheck('RedirectToUpstream self-origin does not loop in browser', page.url().startsWith(`http://${hosts.erp}/`) && body.includes('UpstreamBaseUrl points to the gateway itself'), body.slice(0, 200));

    await writeGatewaySitesConfig();
    await stopIisExpressProcesses();
    spawnIisExpress('GatewayRealBrowser', 'gateway-after-redirect-self');
    spawnIisExpress('MockBackendRealBrowser', 'mock-backend-after-redirect-self');
    await waitForUrl(`http://localhost:${gatewayPort}/healthz.ashx`);
    await waitForUrl(`http://localhost:${backendPort}/mock/erp-main/login`);

    await page.goto(`https://${hosts.erpHttps}/portal`, { waitUntil: 'networkidle' });
    await screenshot(page, 'https-erp-authorized');
    recordCheck('HTTPS gateway endpoint renders through browser', await page.locator('input[name="companyName"]').count() === 0);
